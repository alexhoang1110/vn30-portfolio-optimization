...
def get_estimation_data(rebalance_date, prices_pivot, pit_membership, window_years=2, min_required_days=253):
    ...
    price_window = price_window[price_window.index < rebalance_date]  # exclude the rebalance day 
...
def compute_weights_at_date(rebalance_date, prices_pivot, pit_membership, outstanding_shares, vn30_index, min_eligible=5):
    """
    ...
    # Simplification vs. Week 3: fixed 50% confidence per view instead of a
    ...
    bl = BlackLittermanModel(
        ...
        omega='idzorek', view_confidences=[0.5] * len(viewdict)
    )
...
def run_backtest(rebalance_dates, prices_pivot, pit_membership, outstanding_shares, vn30_index):
    ...
    return pd.DataFrame(portfolio_values, index=dates_log)